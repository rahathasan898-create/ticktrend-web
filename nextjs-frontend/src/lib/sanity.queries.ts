...
export const lessonBySlugsQuery = groq`*[_type == "course" && slug.current == $courseSlug][0] { "lesson": lessons[]->[slug.current == $lessonSlug][0] { _id, title, body }, _id, title, "slug": slug.current, "lessons": lessons[]->{_id, title, "slug": slug.current} }`;
...
// --- FIX: This query now correctly fetches all lesson slugs for each course ---
export const coursePathsQuery = groq`*[_type == "course" && defined(slug.current)]{ "slug": slug.current, "lessons": lessons[defined(slug.current)][].slug.current }`;
